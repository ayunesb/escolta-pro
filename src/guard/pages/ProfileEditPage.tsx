...
export default function ProfileEditPage() {
  ...
  const onSubmit = async (f: Form) => {
    ...
    if (f.photo_file && f.photo_file[0]) {
      const path = `${user.id}/avatar-${Date.now()}.jpg`;
      await supabase.storage.from("profiles").upload(path, f.photo_file[0], { upsert: false });
      // get signed view url via edge function
      const resp = await fetch("/functions/v1/signed_url", {
        method: "POST",
        headers: {
      ...
      const { url } = await resp.json();
      setAvatar(url);
      await supabase.from("profiles").update({ photo_url: path }).eq("id", user.id); // store path only
    }

    // 2) KYC uploads (front/back)
    async function uploadKyc(file?: File, label?: string): Promise<string | undefined> {
      if (!file || !label) return undefined;
      const path = `${user.id}/kyc-${label}-${Date.now()}.jpg`;
      await supabase.storage.from("kyc").upload(path, file, { upsert: false });
      return path;
    }
    const idFrontPath = await uploadKyc(f.id_front && f.id_front[0], "front");
    const idBackPath  = await uploadKyc(f.id_back && f.id_back[0], "back");
  ...
  return (
    <div className="p-4 space-y-4">
      ...
      <Card className="p-4 space-y-3">
        ...
        <div>
          <Label>Foto</Label>
          <Input type="file" accept="image/*" {...register("photo_file")} />
        </div>
        <div className="grid grid-cols-2 gap-3">
          <div>
            <Label>Nombre</Label>
        ...
        <div>
          <Label>Teléfono (E.164)</Label>
          <Input placeholder="+521234567890" {...register("phone_e164")} />
        </div>
        <div className="grid grid-cols-3 gap-3">
          <div>
            <Label>Tarifa por hora (MXN)</Label>
            <Input type="number" min={0} {...register("hourly_rate_mxn_cents", { valueAsNumber: true })} />
          </div>
          <div>
            <Label>¿Disponible armado?</Label>
            <Input type="checkbox" {...register("armed_available")} />
          </div>
          <div>
            <Label>¿Disponible con vehículo?</Label>
            <Input type="checkbox" {...register("vehicle_available")} />
          </div>
        </div>
      </Card>
      ...
      <Card className="p-4 space-y-3">
        ...
        <div>
          <Label>INE/ID – Frente</Label>
          <Input type="file" accept="image/*,application/pdf" {...register("id_front")} />
        </div>
        <div>
          <Label>INE/ID – Reverso</Label>
          <Input type="file" accept="image/*,application/pdf" {...register("id_back")} />
        </div>
        <p className="text-xs text-muted-foreground">Los archivos se guardan de forma privada. Blindado usa URLs firmadas por 1 hora.</p>
      </Card>
